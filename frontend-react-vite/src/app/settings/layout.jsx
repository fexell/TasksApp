...
export default function SettingsLayout({ children }) {
  ...
  useEffect(() => {
    if (isInitialized && !isAuthenticated && !isLoggingOut) {
      navigate("/auth/protected?from=guard")
    }
  }, [isAuthenticated, isInitialized, isLoggingOut, navigate])
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 sm:py-14">
        <div className="flex items-center justify-between mb-10 py-4 sticky top-0 bg-neutral-950">
          <div className="flex items-center gap-2 font-mono text-xs tracking-widest text-neutral-500">
            ...
            <Link to="/" className="hover:underline text-white hover:text-(--primary-color)">{(import.meta.env.VITE_APP_NAME ?? "ENTRY").toUpperCase()}</Link> — ACCOUNT SETTINGS
          </div>
